...
class Message {
  ...
  static all(callback) {
    connection.query(
      ...
      // 2eme arg : callback qui prend en paramétre 1. une erreur s'il
      // y en a une et 2. le nb de lignes retournées
      (err, rows) => {
        if (err) throw err;
        // Sinon tt s'est bien passé on éxecute la callback
  ...
  static find(id, callback) {
    connection.query(
      ...
      [id],
      // 3eme arg : callback qui prend en paramétre 1. une erreur s'il y
      // en a une et 2. le nb de lignes retournées
      (err, rows) => {
        if (err) throw err;
        // Sinon tt s'est bien passé on éxecute la callback
        // On map sur le résultat pour que chaque message récupéré en base soit une
